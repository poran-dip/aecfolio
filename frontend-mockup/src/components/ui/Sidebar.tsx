...
export function Sidebar({ role, userName, userEmail, userImage }: SidebarProps) {
  ...
  // Mockup dynamic role detection based on url
  let currentRole = role;
  if (pathname?.startsWith("/faculty")) currentRole = "FACULTY";
  if (pathname?.startsWith("/admin")) currentRole = "ADMIN";

  const navItems =
    currentRole === "FACULTY" ? facultyNav : currentRole === "ADMIN" ? adminNav : studentNav;

  const roleLabel =
    currentRole === "FACULTY" ? "Faculty Advisor" : currentRole === "ADMIN" ? "Administrator" : "Student";
