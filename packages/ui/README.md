...
The `.cv-sheet` sheet is one long page, which is right for the `preview` script and wrong for a CV that runs to several pages. The app's live preview does not use it: it asks the template for its margins (`template.pageMargins`, built from the same `Density` record as the `@page` rule) and `pageBox` turns them into the size of a sheet and of the area inside its margins. Each page is then one column of a multi-column layout that is exactly that tall, so the browser breaks it the way it breaks the PDF, entries kept whole and headings kept with what follows. `pageBox` rounds the printable height up to a whole pixel because Chromium does when it prints, and a preview that used the nominal height would push a last entry to the next page that the PDF fits on this one. The worker's pagination test measures that against `page.pdf`, so a Chromium upgrade that changes it fails there.
