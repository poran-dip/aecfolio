...
- `cv_preferences` — one row per `(student, template)`, unique on that pair. `sections` (jsonb) holds the student's saved defaults: `[{ type, include, order, entryOrder: [id, ...], ...options }, ...]`. `entryOrder` applies to every section type, built-in and custom alike, and to the socials list, with one exception: `results` ignores it, because semesters always print first to last. A stored `entryOrder` on a `results` section is harmless and simply not read.
